...
int main() {
    ...
    while (true) {
        int currentMinArrivalTimeProcessIndex = -1;
        ...
        for (int i = 0; i < numOfProcesses; i++) {
            if (processes[i].remainingTime > 0) {
                ...
                if (processes[i].arrivalTime < currentMinArrivalTime) {
                    ...
                    currentMinArrivalTimeProcessIndex = i;
                }
            }
        }
        ...
        Process& cp = processes[currentMinArrivalTimeProcessIndex];
